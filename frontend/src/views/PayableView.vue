...
import {
  fetchPayableList, createPayable, recordPayablePayment,
  fetchPayablePayments, markPayableOverdue, batchPayableOverdue,
  deletePayable,
  type PayableVO, type PayableForm, type PaymentRecordForm, type PaymentRecordVO
} from '@/api/payable'
...
async function loadList() {
  loading.value = true
  try { list.value = (await fetchPayableList(keyword.value || undefined, statusFilter.value || undefined)).data.data ?? [] }
...
const formRef = ref()
const formData = reactive<PayableForm>({ code: '', counterpartyId: undefined, contractId: undefined, amount: undefined, dueDate: '', remark: '' })
const formRules = {
  code: [{ required: true, message: '请输入单据编号', trigger: 'blur' }, { max: 50, message: '最多 50 字符', trigger: 'blur' }],
  counterpartyId: [{ required: true, message: '请选择供应商', trigger: 'change' }],
...
async function handleSubmit() {
  ...
  try { await createPayable(formData); ElMessage.success('创建成功'); dialogVisible.value = false; await loadList() }
  finally { submitting.value = false }
}

const payDialogVisible = ref(false)
const payingRow = ref<PayableVO | null>(null)
const payForm = reactive<PaymentRecordForm>({ amount: undefined, payDate: '', remark: '' })

function openPay(row: PayableVO) {
  payingRow.value = row
...
async function handlePay() {
  ...
  await recordPayablePayment(payingRow.value!.id, payForm)
  ElMessage.success('登记成功'); payDialogVisible.value = false; await loadList()
}

const historyDialogVisible = ref(false)
const historyRow = ref<PayableVO | null>(null)
const historyRecords = ref<PaymentRecordVO[]>([])
const historyLoading = ref(false)

async function openHistory(row: PayableVO) {
  historyRow.value = row; historyLoading.value = true; historyDialogVisible.value = true
  try { historyRecords.value = (await fetchPayablePayments(row.id)).data.data ?? [] }
  finally { historyLoading.value = false }
}

async function handleOverdue(row: PayableVO) {
  await ElMessageBox.confirm(`确认将「${row.code}」标记为逾期？`, '操作确认', { type: 'warning' })
  await markPayableOverdue(row.id); ElMessage.success('已标记逾期'); await loadList()
}

async function handleBatchOverdue() {
  await ElMessageBox.confirm('将所有已过到期日但未结清的应付标记为逾期？', '批量逾期', { type: 'warning' })
  const res = await batchPayableOverdue()
  ElMessage.success(`完成，共标记 ${res.data.data?.affected ?? 0} 笔`); await loadList()
}

async function handleDelete(row: PayableVO) {
  await ElMessageBox.confirm(`确认删除「${row.code}」？`, '删除确认', { type: 'warning' })
  await deletePayable(row.id); ElMessage.success('删除成功'); await loadList()
}
...
<template>
  <div class="page">
    <div class="page-header">
      <div><h2>应付账款</h2><p class="sub">管理供应商欠款 — 登记付款、跟踪逾期</p></div>
      <div style="display:flex;gap:10px">
        <el-button @click="handleBatchOverdue">批量标记逾期</el-button>
        <el-button type="primary" :icon="Plus" @click="openCreate">新增应付</el-button>
      </div>
    </div>
      ...
      <el-form :inline="true">
        ...
        <el-form-item label="状态">
          <el-select v-model="statusFilter" clearable placeholder="全部" style="width:120px">
            <el-option label="待付" value="pending" /><el-option label="部分" value="partial" />
    ...
    <div class="summary-cards">
      <el-card shadow="never" class="sc"><div class="sl">应付总额</div><div class="sv">¥{{ fmt(totalAmount) }}</div></el-card>
      <el-card shadow="never" class="sc"><div class="sl">已付</div><div class="sv" style="color:#67c23a">¥{{ fmt(totalPaid) }}</div></el-card>
      <el-card shadow="never" class="sc"><div class="sl">待付</div><div class="sv" style="color:#e6a23c">¥{{ fmt(totalRemaining) }}</div></el-card>
    ...
    <el-card shadow="never">
      <el-table :data="list" v-loading="loading" stripe>
        ...
        <el-table-column prop="counterpartyName" label="供应商" width="120" />
        ...
        <el-table-column label="待付" width="130" align="right"><template #default="{row}"><span style="color:#e6a23c">¥{{ fmt(row.remaining) }}</span></template></el-table-column>
        ...
        <el-table-column label="操作" width="260" align="center" fixed="right">
          <template #default="{row}">
            ...
            <el-button v-if="row.status!=='paid'" link type="primary" size="small" @click="openPay(row)">登记付款</el-button>
    ...
    <el-dialog v-model="dialogVisible" title="新增应付" width="500px" destroy-on-close>
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="90px">
        <el-form-item label="单据编号" prop="code"><el-input v-model="formData.code" maxlength="50" /></el-form-item>
        <el-form-item label="供应商" prop="counterpartyId"><el-select v-model="formData.counterpartyId" filterable style="width:100%"><el-option v-for="c in cpOptions" :key="c.id" :label="c.name" :value="c.id" /></el-select></el-form-item>
        ...
        <el-form-item label="应付金额" prop="amount"><el-input-number v-model="formData.amount" :min="0.01" :precision="2" :controls="false" style="width:100%" /></el-form-item>
    ...
    <el-dialog v-model="payDialogVisible" title="登记付款" width="420px" destroy-on-close>
      <p>单据：{{ payingRow?.code }}，待付余额：¥{{ fmt(payingRow?.remaining ?? 0) }}</p>
      <el-form label-width="80px">
        <el-form-item label="付款金额"><el-input-number v-model="payForm.amount" :min="0.01" :max="payingRow?.remaining" :precision="2" :controls="false" style="width:100%" placeholder="本次付款金额" /></el-form-item>
        <el-form-item label="付款日期"><el-date-picker v-model="payForm.payDate" type="date" value-format="YYYY-MM-DD" style="width:100%" /></el-form-item>
    ...
    <el-dialog v-model="historyDialogVisible" title="付款流水" width="500px" destroy-on-close>
