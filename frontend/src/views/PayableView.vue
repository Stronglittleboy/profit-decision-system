...
import { fetchPayableList, createPayable, recordPayablePayment, markPayableOverdue, deletePayable, type PayableVO, type PayableForm } from '@/api/payable'
...
function openPay(row: PayableVO) { payingRow.value = row; payAmount.value = undefined; payDialogVisible.value = true }

async function handlePay() {
  if (!payAmount.value || payAmount.value <= 0) { ElMessage.warning('请输入有效金额'); return }
  await recordPayablePayment(payingRow.value!.id, payAmount.value)
...
<template>
  <div class="page">
    ...
    <el-dialog v-model="payDialogVisible" title="登记付款" width="400px" destroy-on-close>
      <p>单据：{{ payingRow?.code }}，待付余额：¥{{ fmt(payingRow?.remaining ?? 0) }}</p>
      <el-input-number v-model="payAmount" :min="0.01" :max="payingRow?.remaining" :precision="2" :controls="false" style="width:100%" placeholder="本次付款金额" />
